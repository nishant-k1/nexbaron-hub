    ...
    >
      ...
      >
        <div className="h-16 shrink-0 px-5 flex items-center gap-3">
          <Link to={`/${division}`} className="cursor-pointer flex items-center gap-3 hover:opacity-90">
            <BrandMark size={40} />
            <div>
          ...
          {NAV.map(({ to, label, icon: Icon, end, badge }) => (
            <NavLink key={to} to={to} end={end}
              className={({ isActive }) => cn(
                "cursor-pointer relative flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors",
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="h-14 sm:h-16 bg-neutral-bg flex items-center justify-between px-4 sm:px-6 shrink-0 gap-3 sm:gap-4">
          ...
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-sm min-w-0 overflow-hidden">
            <Link to={`/${division}`} className="cursor-pointer flex items-center gap-1.5 text-muted hover:text-heading transition-colors shrink-0">
                ...
                return (
                  <Fragment key={href}>
                    ...
                    {isLast ? (
                      ...
                      <Link to={href} className="cursor-pointer text-muted hover:text-heading truncate transition-colors" title={raw}>{label}</Link>
                    )}
                  </Fragment>
                )
              })
